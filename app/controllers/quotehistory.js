...
function checkClick(e){
	console.log("quotehistory.js::transform is JSON.stringify(e) ::" +JSON.stringify(e));
}
...
function transformFunction(model) {
        ...
        ///console.log("quotehistory.js::transform is ::" +JSON.stringify(transform));
...
console.log("quotehistory.js::JSON stringify content: "+JSON.stringify(content));

function setDate(e){
	console.log("quotehistory.js::JSON stringify content: "+JSON.stringify(e));
	var date = e.value;
    ...
    console.log("quotehistory.js::setDate: date: "+date+" datesplit: "+datesplit+" datepaid: "+datepaid);
...
function jobDetailAddRow (date,notesbody,imageurl,dateadded,employee) {
		console.log("quotehistory.js::jobDetailAddRow: date: "+date+"  dateadded: "+dateadded+" new Date(+dateadded): "+new Date(+dateadded));
...
//Add row here.
var sid = args.sid;
console.log("quotehistory.js::sid right before key in contents value: "+sid);
console.log("quotehistory.js::content.length: "+content.length);
var paidamount = 0;
...
console.log("quotehistory.js::paidamount : "+paidamount);
//$.labor_table.appendRow(jobrowentry);

function closeWin(e) {
        console.log("quotehistory.js::e is: "+JSON.stringify(e));
}

function UploadPhotoToServer(imagemedia){
        console.log("quotehistory.js::UploadPhotoToServer:: Upload photo to the server.");
        ...
        console.log("quotehistory.js::beginning to upload to the cloud.");
       ...
       // uploadPictoGoogle(image,"uploadphoto3.jpeg");
        ...
        //console.log("quotehistory.js::UploadPhotoToServer::image sid is : " +imagesid);
}

function uploadFile(e){
        console.log("quotehistory.js::JSON stringify e uploadFile : " +JSON.stringify(e));
...
function takePic(e){ 
        console.log("quotehistory.js::JSON stringify e takePic:" +JSON.stringify(e));
        Titanium.Media.showCamera({
                success:function(e){
                        ...
                        console.log("quotehistory::takePic::error:unable to load the camera");
                        alert("error:"+e.code+": Unable to load camera.");
                }, cancel:function(e){
                		alert("error:"+e.code+": Unable to load camera.");
                        console.log("quotehistory::takePic::cancel:unable to load the camera");
                },
                allowEditing:true,
...
var sid = args.sid;
console.log("quotehistory.js::before notes_textarea hintText: JSON.stringify(args): "+JSON.stringify(args)+" sid:"+sid);
$.notes_textarea._hintText = "100";           
$.notes_textarea.addEventListener("blur",function(e){
        console.log("quotehistory.js::JSON.stringify(e)  :" +JSON.stringify(e));
        e.source.keyboardToolbar.items = null;
        var datepaid = e.source.datepaid;
        $.paymentdonebutton.datepaid = datepaid;
        if (datepaid) {
        	console.log("quotehistory.js::before enterNotes(e): JSON.stringify(e): "+JSON.stringify(e));
        	enterNotes(e);
        	e.source.value = "";
        } else {
...
$.paymentdonebutton.addEventListener("click",function(e){
    console.log("quotehistory.js::paymentdone:JSON.stringify(e)  :" +JSON.stringify(e));
 	$.notes_textarea.blur();
});
        
function enterNotes(e,imgurl) {
        console.log("quotehistory.js::JSON.stringify(e) enterNotes  :" +JSON.stringify(e));
        //$.quotehistory_window.show($.notes_textarea);
        //$.quotehistory_window.add(textfield);
        var date = new Date();
        var now = Date.now().toString();
        var jobitemid = now;
        console.log("quotehistory.js:: jobitemid is:"+jobitemid);
        ...
        payment.fetch();
        var content = payment.toJSON();
        console.log("quotehistory.js::JSON stringify payment after write: "+JSON.stringify(content));
        var thedate = date.toString().replace(".","").split(' ',4).toString().replace(/,/g,' ')+' '+Alloy.Globals.formatAMPM(date);
        //console.log("quotehistory.js::thedate is: " +thedate);
        ...
        console.log("quotehistory.js::enterNotes: paidamount , parseFloat(notesbody) : "+paidamount+" + "+parseFloat(notesbody));
 ...
 function submit(thedate,notesbody,imageurl,jobitemid,payment,employee) {  
    ...
    onload: function() {
        try {
	    		...
	    		console.log("quotehistory.js::submit: number of link found: " +link+ " length: "+link.length);
	    		...
	    		Ti.API.info("quotehistory.js::submit: self href is : "+selfhref+" edit href is: "+edithref);
	    		Ti.API.info("quotehistory.js::submit: idtag is : "+idtag);
	    		console.log("quotehistory.js::submit:: update DB with jobitemid :" +jobitemid);
			...
			alert('Modified & Saved Successfully!');
                ...
                Ti.API.info("quotehistory.js::submit::cathing e: "+JSON.stringify(e));
        }     
    },
    onerror: function(e) {
        console.log("quotehistory.js::submit::error e: "+JSON.stringify(e));
...
console.log("quotehistory.js::jsonargs : "+JSON.stringify(args));
var firstname = args.firstname;
var lastname = args.lastname;
var proposalnumber = args.proposalnumber;
var filename = 'project_'+proposalnumber+'_'+firstname+'_'+lastname;
Titanium.App.Properties.setString('filename',filename);
$.paymentsection.headerTitle = firstname+" "+lastname+"    PAID: "+paidamount;
$.notes_textarea.paidamount = paidamount;
console.log("quotehistory.js::value derived from args: proposalnumber: "+proposalnumber+" firstname: "+firstname+" lastname: "+lastname);
...
function getParentFolder(args) {
	...
	var xhr = Ti.Network.createHTTPClient({
	    ...
	    try {
	    		...
	    		console.log("quotehistory.js::args inside getParentFolder: "+JSON.stringify(args));
	    		//var filename = 'test03';
	...
	xhr.onerror = function(e){
		alert("error:"+e.code+": Please connect to the network.");
		console.log("quotehistory::getParentFolder::Unable to connect to the cloud. error is: "+JSON.stringify(e));
	};
...
function createSpreadsheet(filename,parentid) {
	console.log("quotehistory.js::create ss with filename: "+filename+" and parentid: "+parentid);
	var jsonpost = '{'
		 +'\"title\": \"'+filename+'\",'
		 +'\"shared\": \"true\",'
		 +'\"parents\": ['
	    ...
	    try {
	    		...
	    		var json = JSON.parse(this.responseText);
	    		var sid = json.id;
	    		console.log("quotehistory.js::sid : "+sid);
	...
	xhr.onerror = function(e){
		alert("error:"+e.code+": Please connect to the network.");
		console.log("quotehistory::createSpreadsheet::Unable to connect to the cloud. error is: "+JSON.stringify(e));
	};
	xhr.open("POST", 'https://www.googleapis.com/drive/v2/files');	
	xhr.setRequestHeader("Content-type", "application/json");
    xhr.setRequestHeader("Authorization", 'Bearer '+Alloy.Globals.googleAuthSheet.getAccessToken());
    console.log("quotehistory.js::json post: "+jsonpost);
	xhr.send(jsonpost);
}
...
function fileExist(){
	    ...
	    try {
			...
			console.log("quotehistory.js::jsonlist.items.length: "+jsonlist.items.length);
			var filename = Titanium.App.Properties.getString('filename');
			filelist = [];
			if (jsonlist.items.length == "0" ){
				console.log("quotehistory.js::File DOES NOT EXIST");
				var fileexist = "false";
				createSpreadsheet(filename,parentid);  // create file when does not exists
			} else {
				var fileexist = "true";
				console.log("quotehistory.js::quotehistory.js::fileExist:: File exist. sid is: "+jsonlist.items[0].id+" Skipped.");
				populatepaymentSIDtoDB(filename,sid);
			};
		}
		});
	xhr.onerror = function(e){
		alert("error:"+e.code+": Please connect to the network.");
		console.log("quotehistory::fileExist::Unable to connect to the cloud. error is : "+JSON.stringify(e));
	};
...
//console.log("quotehistory.js::create spreadsheet with filename: "+filename+" and parentid: "+parentid); 
//createSpreadsheet(filename,parentid); 
...
console.log("quotehistory.js::paymentsidfile" +paymentsidfile);
//console.log("quotehistory.js::JSON.stringify(paymentsidfilejson)" +paymentsidfilejson);*/
...
function populatepaymentSIDtoDB(filename,sid) {
	...
	thepaymentsid.fetch();
	Ti.API.info(" quotehistory.js::populatepaymentSIDtoDB:: thepaymentsid : "+JSON.stringify(thepaymentsid));
	}

//Retrieve cloud data again

var sid = args.sid;
Ti.API.info("sid for payment in quotehistory.js : "+sid);
Alloy.Globals.getPrivateData(sid,"payment");

function uploadPictoGoogle(image,filename){
	console.log("quotehistory.js::uploadPictoGoogle::create ss with filename: "+filename);
	var base64Data = Ti.Utils.base64encode(image);
	        ...
	        parts.push('--' + bound + '--');
	 		...
	 		var xhr =  Titanium.Network.createHTTPClient({
			    onload: function() {
			    	try {
			    		var json = JSON.parse(this.responseText);
	    				Ti.API.info("quotehistory.js::uploadPictoGoogle::response is: "+JSON.stringify(json));
	    				var id = json.id;
	    				var webcontentlink = json.webContentLink;
	    				Ti.API.info("quotehistory.js::uploadPictoGoogle::id is: "+id+" webcontentlink: "+webcontentlink);
	    				shareAnyonePermission(id);
	    				var e = {"value":"none","source":{"_hintText":id}};
	    				console.log("quotehistory.js::uploadPictoGoogle::entering urlimage with info below e: "+JSON.stringify(e));
	    				enterNotes(e,webcontentlink);
			    	} catch(e){
			    		Ti.API.info("quotehistory.js::uploadPictoGoogle::cathing e: "+JSON.stringify(e));
			    	} 
			    	return id;    
			    },
			    onerror: function(e) {
			    	Ti.API.info("quotehistory.js::uploadPictoGoogle::error e: "+JSON.stringify(e));
			       alert("error:"+e.code+": Please connect to the network.");
			    }
			});
			xhr.open("POST", url);
			...
			xhr.send(parts.join("\r\n"));
			Ti.API.info('done POSTed');
			//Ti.API.info("quotehistory.js::uploadPictoGoogle::sid outside is: "+id);
}

function shareAnyonePermission(sid){
	console.log("quotehistory.js::shareAnyonePermission::sid: "+sid);
	var jsonpost = '{'
		 +'\"role\": \"reader\",'
		 +'\"type\": \"anyone\"'
		+'}';
	    ...
	    onload: function(e) {
	    try {
	    		Ti.API.info("quotehistory.js::shareAnyonePermission::response is: "+this.responseText);
	    	} catch(e){
				Ti.API.info("cathing e: "+JSON.stringify(e));
			}
		}
	...
	xhr.onerror = function(e){
		console.log("quotehistory::shareAnyonePermission::Unable to connect to the cloud. error is :"+JSON.stringify(e));
		alert("error:"+e.code+": Please connect to the network.");
	};
	xhr.open("POST", 'https://www.googleapis.com/drive/v2/files/'+sid+'/permissions');	
	xhr.setRequestHeader("Content-type", "application/json");
    xhr.setRequestHeader("Authorization", 'Bearer '+Alloy.Globals.googleAuthSheet.getAccessToken());
    console.log("quotehistory.js::shareAnyonePermission::json post: "+jsonpost);
	xhr.send(jsonpost);
}

var headertitle = args.title.title.split(':')[0];
console.log("quotehistorys:headerTitle:: " +headertitle);
//$.paymentsection.headerTitle = headertitle;

$.labor_table.addEventListener("delete", function(e){
	console.log("quotehistory.js::$.labor_table delete: "+JSON.stringify(e));
	...
	console.log("quotehistory.js::$.labor_table delete: idtag:"+existingurlsidtag+" selfhref: "+existingurlsselfhref+" edithref: "+existingurlsedithref);
	var xhr = Ti.Network.createHTTPClient({
	    onload: function(e) {
	    try {
	    		Ti.API.info("quotehistory.js::$.labor_table delete:success e: "+JSON.stringify(e));
	    		Ti.API.info("quotehistory.js::$.labor_table delete:response is: "+this.responseText);
	    	} catch(e){
				Ti.API.info("quotehistory.js::$.labor_table delete:cathing e: "+JSON.stringify(e));
			}
		}
	});
	...
	//xhr.setRequestHeader("Content-type", "application/json");
    xhr.setRequestHeader("Authorization", 'Bearer '+Alloy.Globals.googleAuthSheet.getAccessToken());
	if (existingurlsedithref) {xhr.send();} else {console.log("quotehistory.js::$.labor_table delete: NO edithref. abort delete ");}
	console.log("quotehistory.js::$.labor_table delete: DONE: DELETE "+existingurlsedithref);
});

function blurIT(e) {
	console.log("quotehistory.js::blurIT::JSON.stringify(e): "+JSON.stringify(e));
	$.notes_textarea.blur();
}
...
//function updateInvoice(edithref,selfhref,idtag,proposalnumber,clientfirstname,clientlastname,total,bal,paid,lastpaiddate,followupdate,clientphone,clientemail,duedate,currency,status){
function updateInvoice(paidamount,datepaid,balance){ 
		...
		var xmldatastring = '<entry xmlns=\'http://www.w3.org/2005/Atom\' xmlns:gsx=\'http://schemas.google.com/spreadsheets/2006/extended\'>'
						...
						+'<gsx:col1>'+proposalnumber+'</gsx:col1><gsx:col2>'+firstname+'</gsx:col2><gsx:col3>'
       ...
       var xhr =  Titanium.Network.createHTTPClient({
   	   onload: function() {
        try {
                Ti.API.info("quotehistory.js::updateInvoice"+this.responseText); 
        } catch(e){
                Ti.API.info("quotehistory.js::updateInvoice::cathing e: "+JSON.stringify(e));
        }     
    },
    onerror: function(e) {
        console.log("quotehistory.js::updateInvoice::error e: "+JSON.stringify(e));
        alert("error:"+e.code+": Please connect to the network.");
    }
});
